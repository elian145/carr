...
| A-04 | MEDIUM | ✅ **CLOSED.** *(Was: **Duplicated filter logic.** API filtering is SQL (`kk/routes/cars.py`); saved-search alert matching is Python (`kk/listing_filters.py:car_matches_filters`). They already drift (`engine_type` vs `fuel_type`), so alerts fire for listings that search would not return.)* Investigation found the finding **PARTIALLY VALID**: the audit's own cited example (`engine_type` vs `fuel_type` aliasing) traced back to a currently-dormant divergence — reachable via direct API calls but not via any live Flutter code path — while a second, currently-**live** correctness bug was discovered during the same investigation and confirmed to actually fire today: `car_matches_filters()` (`kk/listing_filters.py`) treated `trim="base"` as an additional no-filter sentinel alongside `"any"`, even though "Base" is a real, selectable trim (`lib/features/home/home_page.dart`'s trim list) and the default trim assigned to any listing with no explicit trim chosen (`lib/features/sell/sell_listing_payload.dart`) — so a saved search filtered to `trim=Base` was silently treated as unfiltered by trim, alerting on every trim (Sport, Luxury, ...) that the equivalent live search would never have returned. The genuine architectural risk behind both issues is confirmed real — the two implementations (SQL in `kk/routes/cars.py:get_cars()`, Python in `kk/listing_filters.py:car_matches_filters()`) are independently maintained and can silently drift — but the large "single shared filter implementation" refactor implied by the original finding was determined to be unnecessary and explicitly out of scope for this closure; only the two concrete, confirmed correctness bugs were fixed. Implemented in commit `d0226b0` ("fix: align saved-search filter matching"): (1) the `"base"` sentinel was removed from the trim check, leaving `"any"` as the sole no-filter value, with case-insensitive matching preserved; (2) the `fuel_type`/`engine_type` OR-fallback aliasing (`filters.get("fuel_type") or filters.get("engine_type")` against `car.fuel_type or car.engine_type`) was replaced with two independent checks that mirror the exact current SQL semantics in `kk/routes/cars.py:get_cars()` — `fuel_type` remains comma-separated multi-value, case-insensitive exact match against `car.fuel_type` only (mirroring `Car.fuel_type.ilike(ft)` with no `%` wildcards), and `engine_type` remains single-value, case-sensitive exact match against `car.engine_type` only (mirroring `Car.engine_type == engine_type`), with both required to pass independently (AND, not OR-fallback) when both are supplied. `kk/routes/cars.py` itself, the API contract, and Flutter were not touched. One pre-existing test fixture (`kk/tests/test_be07_saved_search_filter_bounds.py`) was minimally corrected — its literal `trim="base"`/car-`trim="lx"` and `fuel_type="gas"`/default-car-`fuel_type` values had unintentionally been exercising the two now-fixed bugs as if they were correct behavior; only those literal fixture values were changed, no assertions or BE-07 implementation logic were touched. Verified via the repository test suite (no production/staging runtime check performed, and none required — this is a pure correctness/test change with no production deployment dependency): dedicated A-04 tests (`kk/tests/test_a04_filter_matcher_parity.py`) — 15 passed, covering trim "Base" matching, "Sport"/"Luxury" not matching a "Base" filter, "any" remaining unconditional, case-insensitive "Base"/"base"/"BASE", `fuel_type`-only, `engine_type`-only, both supplied simultaneously (including two direct regressions proving the old OR-fallback no longer lets one field satisfy the other's filter), and `fuel_type` multi-value/exact-match/no-substring/no-comma-split-for-`engine_type` semantics; `kk/tests/test_be07_saved_search_filter_bounds.py` — 17 passed; full backend suite (`pytest kk/tests`) — 571 passed, 0 failed; `git diff --check` clean. | `kk/listing_filters.py` vs `kk/routes/cars.py:547-660` |
...
| BE-04 | HIGH | ✅ **CLOSED.** *(Was: **Synchronous broadcast fan-out.** `execute_broadcast` sends FCM to up to 5000 users inside the HTTP request; `kk/notification_broadcast.py:106-128`.)* Investigation confirmed the finding was valid and was already fixed in code prior to this closeout, across two commits: `3ae4b79` ("fix(BE-04): move admin broadcast fan-out off the request thread") and `0d0a9c6` ("fix(BE-04): remove HTTP-side pre-claim in scheduled/process (stuck-sending bug)"). `execute_broadcast()` itself is unchanged — it is still a per-recipient synchronous loop — but it is no longer reachable from the HTTP request thread: `POST /api/admin/notifications/broadcast` ("send now") now persists a durable `ScheduledNotification` row via `create_immediate_broadcast_row()` (`scheduled_at=utcnow()`) and enqueues a new Celery task, `send_immediate_broadcast_task` (`kk/tasks/notification_tasks.py`), via `apply_async(..., ignore_result=True, retry=False)` — the same BE-12 safe-enqueue pattern, with no synchronous fallback if the enqueue itself fails (the row simply stays `pending` for beat's sweep or a retried manual action to pick up later). `GET /api/admin/notifications/scheduled` is now list-only (its old inline "lazy delivery" fallback was removed), and `POST /api/admin/notifications/scheduled/process` now only performs a read-only `list_due_pending_ids()` lookup and enqueues one task per due id, instead of calling `execute_broadcast()` itself. The first commit (`3ae4b79`) introduced a self-contained regression — the HTTP-side `process_scheduled_notifications()` route pre-claimed rows (`pending→sending`) before enqueueing, which made the Celery task's own BE-03 atomic claim always see `rowcount == 0` and permanently strand every row in `sending` without ever calling `execute_broadcast()`; this was caught and corrected in the immediate follow-up commit (`0d0a9c6`), which replaced the pre-claim with the read-only `list_due_pending_ids()` and left the `pending→sending` transition solely to the execution context that is about to broadcast (mirroring the existing send-now and beat-sweep paths). Verified via the repository test suite (no production/staging runtime check performed for this closeout): dedicated BE-04 tests (`kk/tests/test_be04_async_broadcast.py`) — 9 tests re-run in this closeout, all passed, covering send-now being fast/durable with no inline fan-out, correct Celery task enqueue arguments, enqueue failure leaving the row `pending` with no synchronous fallback, `GET /scheduled` performing no processing, `POST /scheduled/process` claiming nothing itself and only enqueueing, a dedicated test that actually runs `send_immediate_broadcast_task.run(row_id)` end-to-end and confirms the broadcast completes (the exact regression test that would have caught the stuck-`sending` bug), and concurrent-claim tests across both entry points; sibling `kk/tests/test_be03_scheduled_notification_atomic_claim.py` and `kk/tests/test_be12_retention_dispatch.py` remain unmodified and passing, since this fix reuses their claim/safe-enqueue primitives unchanged. No further backend, Flutter, database/migration, or admin code change is required. **Known dependency, not part of this closure:** end-to-end production effectiveness — i.e., that a queued broadcast is actually sent in a reasonable time, not merely left `pending` indefinitely — depends on **BE-13**'s Celery worker/beat pair being genuinely live and consuming the queue in production, which remains a separate, open, production-verification-only finding; this closure makes no claim about BE-13's production status. | `kk/notification_broadcast.py` |
| BE-05 | HIGH | ✅ **CLOSED.** *(Was: **Invalid FCM tokens never cleared.** Permanent failures (`NotRegistered`) are logged but `user.firebase_token` is not nulled, so dead tokens are retried forever; `kk/push.py:252-270`.)* Implemented in commit `2851980074dfd5af3e6bdda03ba0f3a7434b4d040` ("fix(BE-05): clear firebase_token on definitive permanent FCM token failure"): `send_push()` (`kk/push.py`) now accepts an optional `user_id` and, only when FCM raises the real `firebase_admin.messaging.UnregisteredError` or `SenderIdMismatchError` (the SDK's own definitive, permanent invalid-token error classes, checked via `isinstance` — not string matching), clears the stored token through a new helper, `_clear_invalidated_token()`, via a single race-safe conditional `UPDATE user SET firebase_token = NULL WHERE id = :user_id AND firebase_token = :token` on its own short-lived connection (`db.engine.begin()`), isolated from the caller's ORM session so a cleanup failure can never roll back unrelated pending work (e.g. `Notification` rows in `execute_broadcast()`'s batched commit loop). The `UPDATE` is a no-op if the token has already changed or been cleared, so a concurrently refreshed token is never clobbered. All other outcomes — transient `QuotaExceededError`, the server-credential `ThirdPartyAuthError`, network/timeout errors, and any unrecognized exception type — leave the token untouched (fail closed). All 6 existing `send_push()` call sites (`kk/notification_broadcast.py`, `kk/routes/admin.py`, `kk/socketio_handlers.py`, `kk/routes/chat.py`, `kk/tasks/alert_tasks.py`, `kk/chat_realtime.py`) now pass `user_id` so cleanup applies consistently; `send_push()`'s return type/semantics are unchanged for every caller. BE-04's Celery architecture, `execute_broadcast()`'s fan-out logic, and `ScheduledNotification` state handling were not touched. No migration, no Flutter, no admin UI, no Firebase configuration change, and no FCM batching/multicast were introduced (explicitly deferred). Verified via the repository test suite (re-run in this closeout; no production/staging runtime check performed): dedicated BE-05 tests (`kk/tests/test_be05_push_token_cleanup.py`) — 18 passed, covering permanent-vs-transient error classification, one invalid token not stopping a broadcast, a cleared token never being retried, a real end-to-end concurrency race proving a newer token survives cleanup, a real 0-row no-op cleanup, logout/admin-purge/account-deletion regression, and direct coverage of all 6 `send_push()` call sites. No further backend, Flutter, database/migration, or admin code change is required. | `kk/push.py` |
...
| BE-13 | MEDIUM | ✅ **CLOSED.** *(Was: `Procfile` defines `worker`/`beat` but `start_render.sh` starts only gunicorn — scheduled notifications depend on manually provisioned Render services (**UNKNOWN**); `Procfile`, `start_render.sh`.)* The original concern was that scheduled-notification delivery depended on Celery worker/beat services whose existence and liveness on Render were **unknown** — `start_render.sh` only ever started Gunicorn, so nothing in the repository proved a worker or beat process actually ran in production. Infra commits (`0a2fce6` "infra: provision Celery worker and beat on Render", `cfbff59`, `925fa3a`, `0a29f31`, `51d5e9b` "fix: finalize BE-13 Frankfurt Celery deployment") provisioned dedicated `carr-worker-fra`/`carr-beat-fra` Render Background Worker services (region `frankfurt`) after discovering the original Oregon-region worker/beat could not reach the Frankfurt-region `carr-redis` instance — a genuine, separate deployment bug found and fixed during this work. Production verification (this closeout) confirmed, from live Render Dashboard evidence (service status, logs, and environment configuration — no secrets exposed): `carr-worker-fra` is **Live**, running the expected `celery -A kk.tasks.celery_app.celery_app worker --concurrency=2 --max-tasks-per-child=50 --max-memory-per-child=200000` command, reaches Celery `ready`, and has both `kk.tasks.notification_tasks.process_due_scheduled_notifications` and `kk.tasks.notification_tasks.send_immediate_broadcast` registered; `carr-beat-fra` is **Live**, running the expected `celery -A kk.tasks.celery_app.celery_app beat` command, and repeatedly emits the `process-due-scheduled-notifications` periodic task roughly every 60 seconds; both services connect to the same `carr-redis` instance (host `red-d9fnbplaeets73ceklag`) and are deployed in the Frankfurt region alongside it. A real, observed end-to-end round trip confirmed actual task delivery and execution, not merely service liveness: beat sent the task at ≈00:23:56 → the worker received it at ≈00:23:56 → the worker reported it `succeeded` at ≈00:23:58, with subsequent periodic executions also received and succeeded; the reported result (`processed=0, sent=0, failed=0`) is a genuine successful no-op (no due scheduled notifications existed at that moment), not an error or evidence of non-execution. Both services are running the deployment verified current in this closeout. No production data was created or altered to manufacture this evidence, and no Render service was modified or deleted. Scheduled notifications (`BE-03`'s atomic claim, `BE-04`'s async broadcast dispatch) can therefore now be relied upon to actually run in production — the dependency `BE-04`'s and `BE-12`'s own closure text explicitly flagged as unresolved is now closed. | `Procfile`, `start_render.sh`, `render.yaml` |
| BE-14 | MEDIUM | ✅ **CLOSED.** *(Was: `/api/analytics/track/message` and `/track/favorite` are **no-op** endpoints the client still calls; `kk/routes/analytics.py`.)* Investigation found the original claim **PARTIALLY VALID / INTENTIONAL-BY-DESIGN**: `track_message()`/`track_favorite()` are confirmed genuine no-ops — both validate the listing and return `{"success": true, "counted": false, "code": "server_bound"}` without ever writing to `ListingAnalytics` — but this is not dead/forgotten code. It is the deliberate, already-shipped output of an earlier anti-gaming fix (commit `5e596a9`, "Fix H-01: Stop gameable listing analytics counters"), which intentionally stopped trusting client-reported `messages`/`favorites` increments (previously a trivial `col + 1` per call, with no dedupe) and rebound those two counters to genuine server-side events instead: `record_buyer_message()` (`kk/listing_metrics.py`, invoked from all 5 real chat-send routes in `kk/routes/chat.py`) and `record_favorite_add()` (invoked from the real favorite-toggle route in `kk/routes/favorites.py`). The two client-facing routes were deliberately kept returning HTTP 200 (not removed) "for mobile compatibility" per their own docstrings. Live Flutter call-site tracing confirmed: `AnalyticsService.trackFavorite()` is still called from 4 sites (`global_listing_card_inner_text.dart`, `production_favorites_page.dart`, `car_details_page_owner.dart`, `featured_listing_card.dart`), always immediately after the real favorite-toggle call that already recorded the metric server-side — harmless and fully redundant, not a correctness gap. `AnalyticsService.trackMessage()` is still called from exactly one live site (`car_details_page_contact.dart`, after a successful WhatsApp handoff to the seller) and remains the only signal the backend could ever receive for that still-shipping, server-unobservable contact path — currently discarded by design, which is a possible future analytics-completeness enhancement but not evidence of dead code. Removing either route would additionally break a currently-passing, unrelated security regression test: `kk/tests/test_token_revocation.py::test_ban_blocks_the_three_get_current_user_bypass_routes` explicitly uses both routes (alongside `/api/blur-image`) as test vehicles for the H-01/H-02 JWT-revocation-on-ban fix, specifically because they are `@jwt_required()`-only routes that never call `get_current_user()`. **No code, test, or client change was made**: removal would require a coordinated backend + Flutter + security-test-rewrite change for zero production/security benefit (the endpoints are inert, rate-limited, auth-gated, and cost nothing to leave in place), and would permanently foreclose the one existing hook for future WhatsApp-contact analytics. Verified via investigation only (static code/call-path tracing, docstring/module-comment review, and git archaeology of commit `5e596a9`); no production/staging runtime check was performed and no code was executed beyond read-only test discovery. | `kk/routes/analytics.py` |
...
| BE-04 | HIGH | ✅ **CLOSED** *(was: synchronous FCM broadcast in request path)* — fan-out moved off the request thread into a Celery task (`send_immediate_broadcast_task`); see §8 for full detail | `kk/notification_broadcast.py:106` |
...
| MT-10 | ✅ **Completed** (BE-13 production verification) — confirmed Render has Redis, Celery worker (`carr-worker-fra`), and beat (`carr-beat-fra`) provisioned, live, and processing a real beat→worker round trip in Frankfurt; see §8 detail | BE-13 — scheduled notifications |
...
| BE-04 | ✅ Done — broadcast fan-out moved to Celery (see §8 detail) | Backend |
| BE-05 | ✅ Done — `firebase_token` cleared on definitive permanent FCM failure (see §8 detail) | Backend |
| BE-13 | ✅ Done — Celery worker + beat provisioned and production-verified on Render (see §8 detail) | Ops |
