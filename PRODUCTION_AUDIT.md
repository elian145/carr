...
| D-05 | MEDIUM | Nullability drift: `account_type`/`dealer_status` are `nullable=False` in the model, `nullable=True` in the migration | `kk/models.py:97-98` vs `c9d8e7f6a5b4:40-43` | DB |
...
| D-09 | MEDIUM | Dealer backfill inserts placeholder data (`"Dealership"`, `"Not provided"`) into production rows | `j7k8l9m0n1p2:180-183` | DB |
| D-10 | LOW | Duplicate reports allowed — no unique on `(reporter_id, car_id)` | `kk/models.py:1119-1173` | DB |
| D-11 | LOW | Relational data in JSON columns (`dealership_phones`, `contact_phones`, `SavedSearch.filters`) — no FK integrity | `kk/models.py` | Backend |
| D-12 | LOW | Soft-delete inconsistency: `kk/view_history.py:31` checks only `is_active`, ignoring `status` | | Backend |
...
|---|---|---|---|
| BE-01 | HIGH | **Unbounded dealer listings.** `dealer_profile` does `.all()` on every public dealer page load | `kk/routes/user.py:1066-1071` |
| BE-02 | HIGH | **N+1 queries** — favorites (`kk/routes/favorites.py:28-41`), recently-viewed (`kk/routes/user.py:569-579`), and analytics (`kk/routes/analytics.py:46-63`) call `to_dict()` without eager loading, lazily fetching images/videos/seller per row. `/api/cars` does this correctly (`kk/routes/cars.py:554-559`) — copy that pattern |
| BE-03 | HIGH | **Scheduled notifications can double-send.** `for row in due: row.status = "sending"; commit()` with no `SELECT ... FOR UPDATE` — two beat workers process the same row | `kk/notification_broadcast.py:206-224` |
...
| BE-06 | MEDIUM | Unbounded `.all()` on `/api/my_listings` | `kk/routes/cars.py:1460-1464` |
| BE-07 | MEDIUM | Saved-search `filters` JSON has no size/depth/key limits | `kk/routes/saved_searches.py:18-21` |
| BE-08 | MEDIUM | `_find_by_filters` loads all of a user's saved searches on every create/sync | `kk/routes/saved_searches.py:29-34` |
| BE-09 | MEDIUM | **Inconsistent response shapes** — `/api/cars` → `{cars, pagination}`; `/cars`, `/api/my_listings`, `/api/analytics/listings` → bare arrays; `/api/cars/<id>` → `{car}`. Plus camelCase `plateType`/`plateCity` duplicating snake_case fields | multiple |
| BE-10 | MEDIUM | ~106 broad `except Exception` handlers in route files, many without `db.session.rollback()`; `kk/routes/chat.py` returns generic 500s **without logging** | `kk/routes/` |
| BE-11 | MEDIUM | Two divergent view metrics: `Car.views_count` (detail GET) vs `ListingAnalytics.views` (track endpoint) | |
| BE-12 | MEDIUM | Celery fails **open** — if Redis is down, `retention_dispatch.py` runs alert tasks inline in the web process; `celery_app.py` falls back to `memory://` | `kk/retention_dispatch.py:12-21` |
...
| BE-15 | LOW | LIKE metacharacters (`%`, `_`) unescaped in brand/model filters — pathological pattern performance | `kk/routes/cars.py:573-575` |
| BE-16 | LOW | Case-sensitive exact match on `condition`/`transmission` filters | `kk/routes/cars.py:594-602` |
| BE-17 | LOW | `list_blocked_users` N+1 (`db.session.get` per block) | `kk/routes/chat.py:1168-1171` |
| BE-18 | LOW | No message-send idempotency — client retries create duplicates | `kk/routes/chat.py` |
| BE-19 | LOW | No OpenAI spend cap beyond a 20/hr rate limit | `kk/ai_service.py:239-325` |
| BE-20 | LOW | Dead code: `validate_ownership` and `secure_headers` decorators are defined but never applied | `kk/security.py:498-538,431-450` |
...
Database: D-05 → D-09 (nullability, FK indexes, upsert races, `profile_picture` width, placeholder backfill cleanup).
