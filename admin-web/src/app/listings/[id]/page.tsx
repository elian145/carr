...
import { fetchListingDetail, updateListingStatus } from "@/lib/api";
...
export default function ListingDetailPage() {
  ...
  async function patch(patch: { is_active?: boolean; is_featured?: boolean; status?: string }) {
    setBusy(true);
    try {
  ...
  return (
    <AsyncPageBody
      ...
      reload={reload}
      actions={
        <Link href="/listings" className="rounded-lg border border-surface-border px-3 py-2 text-sm hover:bg-white/5">
          ← Back
        </Link>
      }
    >
      {(detail) => {
        ...
        return (
          <div className="space-y-8">
            <section className="rounded-xl border border-surface-border bg-surface-card p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-2xl font-bold text-brand-300">{formatPrice(c.price)}</p>
                </div>
              ...
              <dl className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  ...
                  <dd className="mt-1">{c.status} · {c.is_active ? "active" : "inactive"}</dd>
                </div>
                <div>
                ...
                <div>
                  ...
                  <dd className="mt-1">
                    {c.seller?.id ? (
                      <Link href={`/users/${c.seller.id}`} className="text-brand-300 hover:underline">
              ...
              {c.description ? (
                <p className="mt-4 text-sm text-surface-muted whitespace-pre-wrap">{c.description}</p>
              ) : null}
              ...
              {c.images && c.images.length > 0 ? (
                <div className="mt-4 flex flex-wrap gap-2">
                  {c.images.slice(0, 6).map((img, i) => (
                    <img
                      key={i}
                      src={img.image_url.startsWith("http") ? img.image_url : `${process.env.NEXT_PUBLIC_API_BASE}${img.image_url}`}
                      alt=""
                      className="h-20 w-28 rounded-lg object-cover border border-surface-border"
                    />
                  ))}
                </div>
              ) : null}
            ...
            {a ? (
              <section className="grid gap-4 sm:grid-cols-5">
                {[
                  ...
                  <div key={label as string} className="rounded-xl border border-surface-border bg-surface-card p-4">
                    <p className="text-xs text-surface-muted">{label}</p>
                    <p className="mt-1 text-2xl font-semibold">{formatNumber(val as number)}</p>
                  </div>
                ))}
              </section>
            ) : null}
            ...
            <section>
              <h3 className="mb-3 text-lg font-medium">Reports ({detail.reports.length})</h3>
