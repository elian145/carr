...
listing_start = next(
    i for i, line in enumerate(widget_lines) if line.strip().startswith("Widget _buildListingSelection")
)
widgets_block = "\n".join(widget_lines[3:listing_start]).rstrip()
listing_block = "\n".join(widget_lines[listing_start:-1]).rstrip()
...
(OUT / "analytics_page_listing_widgets.dart").write_text(
    "part of 'analytics_page.dart';\n\n"
...
print("Split analytics_page_widgets")
